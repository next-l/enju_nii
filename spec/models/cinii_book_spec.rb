...
describe CiniiBook do
  ...
  it 'should search bibliographic records', vcr: true do
    CiniiBook.search('library system')[:total_entries].should eq 3579
  end
  ...
  it 'should import a bibliographic record', vcr: true do
    ...
    book.isbn_records.first.body.should eq '9784872592542'
    ...
    book.subjects.map(&:term).should include('工業材料')
    book.subjects.map(&:term).should include('燃料電池')
    book.subjects.map(&:term).should include('水素エネルギー')
    book.subjects.map(&:term).should include('シミュレーション')
